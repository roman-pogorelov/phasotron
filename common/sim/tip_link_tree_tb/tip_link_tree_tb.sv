...
module tip_link_tree_tb ();
    ...
    initial begin
        do begin
            @(posedge dn_user_clk);
        end while (!the_tip_dn_link.the_tip_transport.the_aurora_transport.stat_link_up[0]);

        $display("Downsteram link is UP!");
        ...
        tip_ctrl_rd_send(dn_ctrl_in, dn_user_clk, 0, 0, 0, 4);
        do begin
            @(posedge dn_user_clk);
        end while (!(dn_ctrl_out.tvalid & dn_ctrl_out.tready & dn_ctrl_out.tlast));
        @(posedge dn_user_clk);
        $display("Report has come in");

        tip_ctrl_wr_send(dn_ctrl_in, dn_user_clk, 0, 0, TIP_CFG_ADDR_INDIV_ADDR, '{MID_ADDR});
        do begin
            @(posedge dn_user_clk);
        end while (!(dn_ctrl_out.tvalid & dn_ctrl_out.tready & dn_ctrl_out.tlast));
        @(posedge dn_user_clk);
        ...
        tip_ctrl_rd_send(dn_ctrl_in, dn_user_clk, 0, MID_ADDR, 0, 4);
        do begin
            @(posedge dn_user_clk);
        end while (!(dn_ctrl_out.tvalid & dn_ctrl_out.tready & dn_ctrl_out.tlast));
        @(posedge dn_user_clk);
